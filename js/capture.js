...
const get = async (req, res, client) => {
    ...
    if (status === 400) {
        ...
        const items = await order.findOne({paypal_id: {$eq: req.query.id}}, {items: true});
        console.log('capture.get', items);
        await utils.release_basket(client, items);
...
const post = async (req, res, client) => {
    ...
    if (!completed) {
        await utils.release_basket(client, req.body.order.items);
        res.status(500).json({error: "An error occured with Paypal"});
    }
}; exports.post = post;
